namespace OrderW {
    ...
    export class GirdForm extends React.Component<any, {
        ...
        constructor() {
            ...
            this.changeMemoValue = this.changeMemoValue.bind(this);
        ...
        changeMemoValue(e: React.SyntheticEvent) {
            let input: HTMLInputElement = e.target as HTMLInputElement;
            this.state.memo = input.value;
            this.setState({ memo: input.value });
        }
        checkOrder(e: React.FormEvent) {
            ...
            jqPost(this.props.apiOrderPath, { order_list: this.state.order_list, memo: this.state.memo })
                .done(function (data, textStatus, jqXHRdata) {
                    if (data.result) {
                        alert('已成功下單,請耐心等候通知~!')
                    } else {
                ...
                <div>
                    <form className="order-list" onSubmit={this.checkOrder}>
                        ...
                        <textarea cols={3} rows={5}
                            ...
                            onChange={this.changeMemoValue.bind(this) } ></textarea>
